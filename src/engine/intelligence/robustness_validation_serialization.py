"""
Deterministic serialization for the robustness validation report
(Sprint 12D).
...
11Y / 11Z / 12A / 12B / 12C engines).

Serialization scope (intentional, matching the Sprint 11U / 11W / 11X
/ 11Y / 11Z / 12A / 12B / 12C discipline):

The lightweight validation projections are persisted IN FULL:

* :class:`RobustnessValidationReport` — validation identity, the overall
  + per-category statuses, scenario / check / passed / failed / skipped
  counts, the outcome distribution, edge-case coverage, label, metadata,
  rationale, limitations.
* :class:`RobustnessScenarioResult` — name, outcome count, checks,
  edge_case.
* :class:`RobustnessCheckResult` — name, category, status, detail.
* :class:`RobustnessCategorySummary` — category + counts.

Every non-primitive value carries a small type tag
(``__enum__`` + ``__enum_class__`` / ``__dataclass__`` / ``__tuple__``)
so the deserializer reconstructs the EXACT model type. Enums are stored
by their stable member name AND qualified by their class name (because
``RobustnessCheckStatus`` shares member names with no other enum in
this module, but the qualifier is retained for forward safety, matching
the Sprint 12A / 12B convention). The persisted text uses sorted keys
so identical results always produce identical bytes.
...
#: Schema version for persisted robustness-validation documents.
ROBUSTNESS_VALIDATION_SCHEMA_VERSION = 1
...
def serialize_robustness(report: RobustnessValidationReport) -> str:
    """Serialize a :class:`RobustnessValidationReport` to canonical JSON."""

    payload = {
        "schema_version": ROBUSTNESS_VALIDATION_SCHEMA_VERSION,
        "report": _to_json(report),
    }
    return json.dumps(payload, sort_keys=True, ensure_ascii=False)


def serialize_robustness_bytes(report: RobustnessValidationReport) -> bytes:
    """Serialize a robustness validation report to canonical JSON bytes."""

    return serialize_robustness(report).encode("utf-8")
...
def deserialize_robustness(payload: str) -> RobustnessValidationReport:
    """Reconstruct a :class:`RobustnessValidationReport` from JSON text."""

    parsed = json.loads(payload)
    _check_schema(parsed)
    return _from_json(parsed.get("report"), RobustnessValidationReport)


def parse_robustness_header(payload: str) -> dict[str, Any]:
    """
    Cheap header-only parse of a persisted robustness-validation
    document.
...
def _check_schema(parsed: Any) -> None:
    ...
    if version != ROBUSTNESS_VALIDATION_SCHEMA_VERSION:
        raise ValueError(
            f"Unsupported robustness-validation schema version {version!r}; "
            f"supported is {ROBUSTNESS_VALIDATION_SCHEMA_VERSION}.",
        )
...
def canonical_robustness_json(report: RobustnessValidationReport) -> str:
    """Canonical (sorted-key) JSON text for a robustness validation report."""

    return serialize_robustness(report)
...
_ENUMS = {
    "RobustnessCheckStatus": RobustnessCheckStatus,
    "RobustnessCategory": RobustnessCategory,
}

_DATACLASSES = {
    "RobustnessValidationReport": RobustnessValidationReport,
    "RobustnessScenarioResult": RobustnessScenarioResult,
    "RobustnessCheckResult": RobustnessCheckResult,
    "RobustnessCategorySummary": RobustnessCategorySummary,
}


def _to_json(value: Any) -> Any:
    """Recursively encode a robustness-validation model value to JSON-safe form."""
